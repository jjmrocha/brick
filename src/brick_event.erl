...
subscribe(Type, Susbcriber) ->
	gen_event:add_handler(?MODULE, {?MODULE, {Type, Susbcriber}}, [Type, Susbcriber]).
...
%% init/1
init([Type, Susbcriber]) ->
	MonitorRef = erlang:monitor(process, Susbcriber),
    {ok, #state{type=Type, subscriber=Susbcriber, ref=MonitorRef}}.

%% handle_event/2
handle_event({Type, Event}, State=#state{type=Type, subscriber=Susbcriber}) ->
	Susbcriber ! Event,
    {ok, State};
